...
public interface UserRelaRepository extends JpaRepository<UserRela, Long> {
  ...
  @Query("SELECT r.following.id FROM UserRela r WHERE r.follower.id = :viewerId AND r.following.id IN :targetIds")
  Set<Long> findFollowingIdsByViewerAndTargets(@Param("viewerId") Long viewerId, @Param("targetIds") List<Long> targetIds);

  // [MỚI THÊM] Lấy danh sách ID những người đang follow TÔI (viewerId) trong tập targetIds
  @Query("SELECT r.follower.id FROM UserRela r WHERE r.following.id = :viewerId AND r.follower.id IN :targetIds")
  Set<Long> findFollowerIdsByViewerAndTargets(@Param("viewerId") Long viewerId, @Param("targetIds") List<Long> targetIds);
}
