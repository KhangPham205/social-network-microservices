...
/**
 * Tests for {@link AdminUserController} – specifically the API Composition logic in
 * {@code getAllUsersForAdmin}.
 *
 * <p>The user-service calls {@link com.socialnetwork.user_service.client.AuthClient}
 * (HTTP Interface/Feign-like) to fetch credential data (email, roles, status) from
 * {@code auth-service} and merges it with local {@code User} data. In these tests, {@code
 * UserService} is mocked with {@code @MockBean}, so the AuthClient stub is internal to
 * {@code UserServiceImpl}. We control the merged output via the mocked service.
 */
@WebMvcTest(
...
class AdminUserControllerTest {
    ...
    /**
     * Builds an {@link AdminUserViewDto} that simulates the result of the API Composition in
     * {@code UserServiceImpl.getAllUsersForAdmin()}. The dto already merges local profile fields
    ...
    void givenNoFilter_whenGetAllUsers_thenReturnsComposedList() throws Exception {
        ...
        AdminUserViewDto user1 =
                buildComposedDto(1L, "Alice", "alice@sn.com", "alice", AccountStatus.ACTIVE, Set.of("USER"));
        AdminUserViewDto user2 =
                buildComposedDto(2L, "Bob", "bob@sn.com", "bob", AccountStatus.ACTIVE, Set.of("USER", "MODERATOR"));
        ...
        PageVO<AdminUserViewDto> page =
                objectMapper.readValue(
                        body,
                        new TypeReference<PageVO<AdminUserViewDto>>() {});
    ...
    @Test
    @WithMockUser
    @DisplayName("GET /admin?filter=… → filter is forwarded to service and results are paged correctly")
    ...
    void givenNoUsers_whenGetAllUsers_thenReturnsEmptyPage() throws Exception {
        ...
        // --- Act & Assert ---
        mockMvc.perform(get(ADMIN_USERS_URL).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    @Test
    @WithMockUser
    @DisplayName("GET /admin → AuthClient data absent for one user → null email/roles in composed DTO")
    void givenAuthClientReturnsPartialData_whenGetAllUsers_thenNullFieldsAreAccepted() throws Exception {
