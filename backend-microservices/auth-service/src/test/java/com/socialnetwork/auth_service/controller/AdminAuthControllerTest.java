 ...
 * <ul>
 *   <li>When the annotation is re-enabled, the {@code FORBIDDEN} tests will naturally pass.</li>
 *   <li>Until then, the tests marked with {@code @WithMockUser(authorities = "USER:CREATE")}
 *       verify the happy-path service interaction.</li>
 * </ul>
 */
@WebMvcTest(
...
class AdminAuthControllerTest {
    ...
    void givenUserCreateAuthority_whenCreateStaff_thenRequestBodyMappedCorrectly() throws Exception {
        ...
        verify(authService).createStaffAccount(
                org.mockito.ArgumentMatchers.argThat(
                        r ->
                                "moderator_01".equals(r.getUsername())
                                        && "MODERATOR".equals(r.getRoleName())));
    }
    ...
    void givenAdminRole_whenCreateStaff_thenServiceResponseReturned() throws Exception {
        ...
        when(authService.createStaffAccount(any()))
                .thenReturn(new RegisterResponse("Staff created"));
    ...
    @DisplayName("POST /staff with only USER:READ authority → 403 Forbidden (when @PreAuthorize active)")
    ...
    @DisplayName("POST /staff as regular USER role → authService.createStaffAccount called (pre-authorise disabled)")
    ...
    void givenAdminAndCreateAuthority_whenCreateStaff_thenReturns200() throws Exception {
        when(authService.createStaffAccount(any())).thenReturn(new RegisterResponse("Admin created staff"));
