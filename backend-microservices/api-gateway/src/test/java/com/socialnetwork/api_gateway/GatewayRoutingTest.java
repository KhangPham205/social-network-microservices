 ...
 * {@code 404 Not Found} on unmatched paths. Both responses confirm that routing decisions were
...
class GatewayRoutingTest {
    ...
    @Test
    @DisplayName("/api/v1/auth/login is routed to auth-service (502 or 503 because LB cannot connect)")
    ...
    void preflight_fromAllowedOrigin_returnsCorsHeaders() {
        webTestClient
                ...
                .expectStatus()
                .value(
                        status -> assertThat(status).isIn(200, 204))
                .expectHeader()
                .valueMatches(
    ...
    @Test
    @DisplayName("OPTIONS preflight from allowed Vite dev-server origin (localhost:5173) → CORS headers present")
    ...
    void request_fromAllowedOrigin_hasAllowCredentialsTrue() {
        ...
        webTestClient
                ...
                .value(
                        ...
                        v -> assertThat(v).isEqualToIgnoringCase("true"));
    }
