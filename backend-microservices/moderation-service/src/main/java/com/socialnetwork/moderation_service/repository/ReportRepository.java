...
public interface ReportRepository
  ...
  @Query(
      "SELECT r.targetUserId AS id, COUNT(r) AS count FROM Report r GROUP BY r.targetUserId ORDER BY count DESC")
  Page<IdCount> findTopReportedUsers(Pageable pageable);

  // 🔥 Query tìm Content (Post/Comment/Message) bị report nhiều nhất
  @Query(
      "SELECT r.targetId AS id, COUNT(r) AS count FROM Report r WHERE r.targetType = :type GROUP BY r.targetId ORDER BY count DESC")
  ...
  boolean existsByTargetIdAndTargetTypeAndIsBannedBySystemIsNotNull(
