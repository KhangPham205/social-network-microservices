 ...
 * <p>Strategy:
 * <ol>
 *   <li>Spin up an in-process Kafka broker via {@code @EmbeddedKafka}.</li>
 *   <li>Use {@link KafkaTemplate} (auto-configured) to publish a raw {@link ModerationActionEvent}
 *       JSON message to the {@code moderation-actions} topic.</li>
 *   <li>Mock both {@link com.socialnetwork.media_service.service.PostService} and {@link com.socialnetwork.media_service.service.CommentService}.</li>
 *   <li>Assert that the listener routes the event correctly and calls the right service method.</li>
 * </ol>
 *
...
class ModerationActionEventListenerIntegrationTest {
    ...
    /**
     * Publishes a {@link ModerationActionEvent} as JSON to the embedded Kafka topic and returns
     * after giving the async listener time to process it.
     */
    private void publishAndAwait(ModerationActionEvent event) throws Exception {
        String json = objectMapper.writeValueAsString(event);
        kafkaTemplate.send(MODERATION_ACTIONS_TOPIC, event.getTargetId(), json).get();
    ...
    @Test
    @DisplayName("BLOCK event for a COMMENT → commentService.updateSystemBanStatus(id, true) is called")
